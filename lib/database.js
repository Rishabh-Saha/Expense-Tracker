...
function getDb() {
  if (!db) {
    ...
    `);
    // One-time migrations
    const ran = (key) => getDb().getFirstSync('SELECT key FROM _migrations WHERE key = ?', [key]);
    const mark = (key) => getDb().runSync('INSERT OR IGNORE INTO _migrations (key) VALUES (?)', [key]);
    if (!ran('add_utilization_cols')) {
      db.execSync(`
        ALTER TABLE statements ADD COLUMN credit_limit REAL;
        ALTER TABLE statements ADD COLUMN statement_balance REAL;
      `);
      mark('add_utilization_cols');
    }
    db.execSync(`
...
export function saveStatement({ filename, month, cardName, creditLimit, statementBalance }) {
  const database = getDb();
  const id = generateId();
  database.runSync(
    'INSERT INTO statements (id, filename, month, card_name, credit_limit, statement_balance, uploaded_at) VALUES (?, ?, ?, ?, ?, ?, ?)',
    [id, filename, month, cardName ?? null, creditLimit ?? null, statementBalance ?? null, Date.now()]
  );
  return id;
}

export function updateStatementMetadata(filename, { creditLimit, statementBalance }) {
  getDb().runSync(
    'UPDATE statements SET credit_limit = ?, statement_balance = ? WHERE filename = ?',
    [creditLimit ?? null, statementBalance ?? null, filename]
  );
}

export function getUtilizationData() {
  // Latest statement per card that has a credit limit
  return getDb().getAllSync(`
    SELECT card_name, credit_limit, statement_balance, month
    FROM statements
    WHERE credit_limit IS NOT NULL AND credit_limit > 0
    ORDER BY month DESC
  `).reduce((acc, row) => {
    // keep only the most recent entry per card
    if (!acc.find(r => r.card_name === row.card_name)) acc.push(row);
    return acc;
  }, []);
}
