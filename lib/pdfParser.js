...
import { PDF_CLAUDE_FALLBACK } from '../constants/models';
...
Rules:
- cardName: identify the bank and card type from the statement header (e.g. "Chase Sapphire Reserve", "Amex Gold", "TD Visa Infinite"). Use "Unknown Card" if not identifiable.
...
export async function extractTransactionsFromPDF(fileUri) {
  ...
  // PDF parsing uses Anthropic API regardless — OpenAI doesn't accept PDF bytes
  const pdfModelId = selectedModel.provider === 'anthropic'
